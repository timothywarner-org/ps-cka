...
- `exercise-files/` -- All course content, organized as `course-NN-topic/mNN-module-name/`. **Mostly scaffolding today**: most module directories hold only `.gitkeep`; the first populated module is `course-03-lifecycle-upgrades/m03-helm-kustomize-crds/` (has a Kustomize base/overlays demo). Manifests get written as each course is recorded.
