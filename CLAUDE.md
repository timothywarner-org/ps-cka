...
- **Canonical authoring hub:** a local Dropbox folder outside this repo, `<AUTHORING_HUB>/CKA-Skill-Path/course-NN-*` (machine-local path, deliberately not recorded here because this repo is public). Shipped runbooks, decks, outlines, and demo scripts are kept there. When populating the repo, copy learner-facing assets (scripts, manifests, runbooks, diagrams) and leave decks/outlines/internal reports behind. **Authority = what was actually recorded** (verify against the demo runbook and the on-rails lab driver), not a later `demos.zip` repackaging.
