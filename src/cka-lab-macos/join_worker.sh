#!/bin/bash
#================================================================
# join_worker.sh — Run on each worker VM to join the kubeadm cluster
#
...
#
# Usage (from your Mac, for each worker):
#   vagrant ssh worker1 -c 'bash /vagrant/join_worker.sh'
#   vagrant ssh worker2 -c 'bash /vagrant/join_worker.sh'
#
# Or SSH directly on the lab network:
#   ssh vagrant@192.168.50.11 'bash /vagrant/join_worker.sh'
#
# Prereqs:
#   - bootstrap_cp.sh has already been run on control1
#   - SSH from worker1/worker2 to control1 (192.168.50.10) works
#     (Vagrant distributes the insecure key, or use password 'vagrant')
#================================================================
